...
@Injectable()
export class HctraMatNativeDateAdapter extends HctraMatDateAdapter<Date> {

    /** Whether to clamp the date between 1 and 9999 to avoid IE and Edge errors. */
    private readonly internalClampDate: boolean;
    ...
    constructor(
        @Optional() @Inject(MAT_DATE_LOCALE) public matDateLocale: string,
        ...
        this.internalClampDate = platform.TRIDENT || platform.EDGE;
    }
    ...
    public getMonthNames(style: 'long' | 'short' | 'narrow'): string[] {
        if (SUPPORTS_INTL_API) {
            ...
            return range(12, i =>
                this.stripDirectionalityCharacters(this.internalFormat(dtf, new Date(2017, i, 1))));
        }
    ...
    public getDateNames(): string[] {
        if (SUPPORTS_INTL_API) {
            ...
            return range(31, i => this.stripDirectionalityCharacters(
                this.internalFormat(dtf, new Date(2017, 0, i + 1))));
        }
    ...
    public getYearName(date: Date): string {
        if (SUPPORTS_INTL_API) {
            ...
            return this.stripDirectionalityCharacters(this.internalFormat(dtf, date));
        }
    ...
    public format(date: Date, displayFormat: { [key: string]: any }): string {
        ...
        if (SUPPORTS_INTL_API) {
            ...
            if (this.internalClampDate && (date.getFullYear() < 1 || date.getFullYear() > 9999)) {
    ...
    public toIso8601(date: Date): string {
        return [
            date.getUTCFullYear(),
            this.padWith2Digit(date.getUTCMonth() + 1),
            this.padWith2Digit(date.getUTCDate())
        ].join('-');
    }
     ...
     */
    private padWith2Digit(n: number) {
        return ('00' + n).slice(-2);
    }
     ...
     */
    private internalFormat(dtf: Intl.DateTimeFormat, date: Date) {
