...
describe('MatCarouselComponent', () => {
  ...
  describe('@Output(change) and @Output(animationStart)', () => {
    beforeEach(() => {
        spyOn(component.changeIndex, 'emit');
    ...
    it('should emit when slideTo is called', fakeAsync(() => {
      ...
      tick();
        ...
        expect(component.changeIndex.emit).toHaveBeenCalledWith(idx);
    }));
    ...
    it('should emit when next is called', fakeAsync(() => {
      ...
      expect(component.animationStart.emit).toHaveBeenCalledWith(1);
        expect(component.changeIndex.emit).toHaveBeenCalledWith(1);
    }));
    ...
    it('should emit when previous is called', fakeAsync(() => {
      ...
      expect(component.animationStart.emit).toHaveBeenCalledWith(component.slidesList.length - 1);
        expect(component.changeIndex.emit).toHaveBeenCalledWith(component.slidesList.length - 1);
    }));
